...
const doJoinTeam = async () => {
  ...
  const res: ApiResponse<boolean> = await myAxios.post("/team/join", {
...
const doQuitTeam = async (id: number) => {
  const res: ApiResponse<boolean> = await myAxios.post("/team/quit", {
    teamId: id,
  });
  if (res?.code === 0) {
    Toast.success("操作成功");
...
const doDeleteTeam = async (id: number) => {
  const res: ApiResponse<boolean> = await myAxios.post("/team/delete", {
    id,
  });
  if (res?.code === 0) {
    Toast.success("操作成功");
